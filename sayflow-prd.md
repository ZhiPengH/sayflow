...
> **范围**：在 macOS 上复刻 SayFlow 的核心功能（语法批改），并新增两项个人功能（Obsidian 写入 + 弹窗跟随鼠标）。
...
用户在任意 App 选中英文文本
        ↓
触发 SayFlow / 言顺（划词热区 / ⌃⌘S 快捷键）
        ↓
SayFlow 通过 macOS Accessibility API 读取选中内容
        ↓
...
**复刻版只做语法批改，不做翻译模式、不做解释模式**——这是 v0.3 收敛的核心决策。原版 SayFlow 本身就是单一用途的语法批改器，强行加多模式会破坏产品定位。如未来确实需要翻译/解释，应作为独立 App 或 v2.0 大版本规划，不混进 v1.0。
...
存储位置：`~/Library/Application Support/SayFlow/prompts.json`，明文 JSON，方便外部备份。
...
```
┌────────────────────────────────────────────────┐
│ 📌 SayFlow     The market are unpredict...   ×  │  顶部栏：图钉、应用名、原文截短预览、关闭
├────────────────────────────────────────────────┤
│                                                 │
...
> 我在阅读英文资料时，遇到一个值得收藏的句子或语法点，触发 SayFlow 看完 AI 解释后，**一键把这条记录追加到我 Obsidian 里指定的 Markdown 文件**，无需切换 App、无需手动粘贴。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| 目标 Markdown 文件 | 文件选择器（限 .md，绝对路径） | 例：`/Users/zhipeng/Obsidian/MyVault/02-Inbox/SayFlow-Inbox.md`。**单文件追加，不需要 Vault 概念**——用户自己挑文件位置即可 |
...
- **文件不存在**：自动创建（含上层目录），并写入一个 H1 标题如 `# SayFlow Inbox`
...
| Q2 | 触发方式？快捷键默认值？ | 划词 + 全局快捷键并行，默认 `⌃⌘S` |
...
志鹏提供了原版 SayFlow 的真实截图后，发现原版是**单一用途的语法批改器**——根本没有"翻译"和"解释"两种模式，所谓"翻译"只是修改后句子的中文 gloss 自动跟在 Corrected 下面。基于这个发现做了如下收敛：
