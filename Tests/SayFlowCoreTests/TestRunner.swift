...
struct SayFlowCoreTestRunner {
    static func main() {
        let tests: [TestCase] = [
            ...
            TestCase(name: "Loopback HTTP endpoints", run: ProviderTests.endpointNormalizerAllowsHTTPLoopbackEndpoints),
            TestCase(name: "HTTP rejected for non-loopback hosts", run: ProviderTests.endpointNormalizerRejectsHTTPForNonLoopbackHosts),
            ...
            TestCase(name: "Provider settings validation", run: ProviderTests.providerSettingsValidationRequiresHTTPSExceptLoopbackBaseURLAndModel),
            TestCase(name: "Translation mode request body", run: ProviderTests.requestFactorySwitchesShortPhrasesToTranslationMode),
            TestCase(name: "DeepSeek requests disable thinking", run: ProviderTests.deepSeekRequestsDisableThinkingForFastInteractiveResults),
            ...
            TestCase(name: "Provider connection probe is minimal", run: ProviderConnectionTestRequestFactoryTests.buildsMinimalNonStreamingConnectionProbe),
            ...
            TestCase(name: "Prompt store private permissions", run: PromptTemplateTests.promptStoreRestrictsExistingFilePermissionsToOwner),
            ...
            TestCase(name: "Translation mode JSON", run: StreamingCorrectionAccumulatorTests.decodesTranslationModeSnapshots),
            ...
            TestCase(name: "Settings store private permissions", run: AppSettingsTests.settingsStoreRestrictsExistingFilePermissionsToOwner),
            ...
            TestCase(name: "Text capture runtime copy fallback action", run: TextCaptureResolverTests.runtimeAccessibilityMissRequestsCopyShortcutFallback),
            TestCase(name: "Text capture sample skips copy fallback action", run: TextCaptureResolverTests.sampleTextMissDoesNotRequestCopyShortcutFallback),
            TestCase(name: "Text capture WeChat hot zone copy fallback action", run: TextCaptureResolverTests.weChatHotZoneAccessibilityMissAfterDragRequestsCopyFallback),
            TestCase(name: "Text capture WeChat hot zone fallback requires drag", run: TextCaptureResolverTests.weChatHotZoneFallbackRequiresDragGesture),
            TestCase(name: "Text capture non-WeChat hot zone skips copy fallback", run: TextCaptureResolverTests.nonWeChatHotZoneMissDoesNotProbeClipboard),
            ...
            TestCase(name: "Selection hot zone shows short translation candidates", run: SelectionHotZonePolicyTests.showsForShortTranslationCandidates),
            ...
            TestCase(name: "Translation mode single word", run: TranslationModePolicyTests.singleEnglishWordUsesTranslationMode),
            TestCase(name: "Translation mode hyphen phrase", run: TranslationModePolicyTests.hyphenatedThreeWordPhraseUsesTranslationMode),
            TestCase(name: "Translation mode four words stays grammar", run: TranslationModePolicyTests.fourWordsStayInGrammarMode),
            ...
            TestCase(name: "Accept closes panel before paste", run: AcceptReplacementFallbackTests.clipboardFallbackClosesPanelBeforeSchedulingPaste),
            TestCase(name: "Accept direct success skips fallback side effects", run: AcceptReplacementFallbackTests.successfulAttemptSkipsClipboardFallbackSideEffects),
            TestCase(name: "Accept failed paste scheduling is reported", run: AcceptReplacementFallbackTests.failedPasteSchedulingIsReported),
            ...
            TestCase(name: "AX direct selection wins", run: AccessibilitySelectedTextPolicyTests.directSelectionWinsWhenNonempty),
            TestCase(name: "AX empty direct selection uses marker", run: AccessibilitySelectedTextPolicyTests.emptyDirectSelectionFallsBackToMarker),
            TestCase(name: "AX empty selection sources return nil", run: AccessibilitySelectedTextPolicyTests.emptySourcesProduceNoSelection),
            TestCase(name: "Deferred paste requires target focus", run: DeferredPasteFocusPolicyTests.pastesOnlyWhenTargetIsFrontmost),
            TestCase(name: "Deferred paste retries activation", run: DeferredPasteFocusPolicyTests.retriesWhileActivationIsPending),
