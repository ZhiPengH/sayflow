...
enum HotKeyParserTests {
    static func parsesSymbolAndWordBasedOptionShortcuts() throws {
        try expectEqual(HotKeyParser.parse("⌥G"), .defaultOptionG)
        try expectEqual(HotKeyParser.parse("Option+G"), .defaultOptionG)
        try expectEqual(HotKeyParser.parse("option g"), .defaultOptionG)
        try expectEqual(
