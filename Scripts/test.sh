...
grep -q 'VERSION="${VERSION:-1.2.8}"' Scripts/build_app.sh
grep -q 'VERSION="${VERSION:-1.2.8}"' Scripts/package_dmg.sh
grep -q 'VERSION="${VERSION:-1.2.8}"' Scripts/verify_package.sh
grep -q 'ALLOW_ADHOC_SIGNATURE="${ALLOW_ADHOC_SIGNATURE:-0}"' Scripts/verify_package.sh
grep -q 'Version 1.2.8' Sources/SayFlowCore/Localization.swift
